<?php
use App\Models\Expertise;

use function Livewire\Volt\{layout, state,computed, title};

title('Home');


$expertises=computed(function(){

    return Expertise::all();

});


?>

<div>

    <!-- ========================== Banner Five Start ========================= -->
    <section class="px-6 bg-white banner-five banner">
        <div class="w-full container-two ">
            <div class="grid-cols-2 gap-4 gird">
                <div class="col-span-1">
                    <div class="banner-inner position-relative">
                        <div class="banner-content">
                            <h1 class="banner-content__title ">{{__('brand.landing_title')}}</h1>
                            <p class="banner-content__desc font-18">{{__('brand.landing_description')}}</p>
                            <div class="banner-content__button">
                                <a href="{{route('about')}}" class="btn btn-main fw-normal"> {{__('brand.more')}}
                                    <span class="icon-right"> <i class="fas fa-arrow-right"></i> </span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="order-1 col-span-1 p-4 order-lg-0">
                    <div class="banner-five__thumb">
                        <img src="/images/house/houseLanding.png" alt="">
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- ========================== Banner Five End ========================= -->
    <!-- ======================= About five Start ========================= -->
    <section class="px-6 about-five padding-y-120">
        <div class="container container-two">
            <div class="flex flex-wrap">
                <div class="order-last lg:order-first md:w-1/2 lg:w-5/12">
                    <div class="about-five__thumbs">
                        <div class="flex flex-wrap gap-y-4">
                            <div class="w-1/2 sm:w-1/2">
                                <div class="flex h-full about-five__thumb one">
                                    <img src="/images/house/house2.jpg" alt="Image">
                                </div>
                            </div>
                            <div class="w-1/2 sm:w-1/2">
                                <div class="about-five__thumb two">
                                    <img src="/images/house/1.png" alt="Image">
                                </div>
                                <div class="about-five__thumb three">
                                    <img src="/images/house/projetImage.jpg" alt="Image">
                                </div>
                            </div>
                        </div>
                        <div class="inline-flex items-center about-five__content">
                            <span class="about-five__icon"><i class="fas fa-users"></i></span>
                            <div class="about-five__texts">
                                <h5 class="mb-0 text-white about-five__amount">10+</h5>
                                <span class="text-white about-five__text text-[18px]">{{__('brand.project_realiser')}}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="px-6 md:w-1/2 lg:w-7/12 lg:pl-20">
                    <div class="about-contents">
                        <div class="text-left section-heading">
                            <span class="section-heading__subtitle">
                                <span class="font-semibold text-gradient">{{__('brand.about_us')}} </span>
                            </span>
                            <h2 class="text-2xl lg:text-4xl section-heading__title">

                                {{__('brand.about_title')}}</h2>

                            <ul class="mt-4 lg:mt-5 check-list">
                                <li class="flex items-center check-list__item">
                                    <span class="icon"><i class="fas fa-check"></i></span>
                                    <span class="text-base font-normal text-gray-700 ">
                                        {{__('brand.about_desc_1')}} </span>
                                </li>
                                <li class="flex items-center check-list__item">
                                    <span class="icon"><i class="fas fa-check"></i></span>
                                    <span class="text-base font-normal text-gray-700 ">
                                        {{__('brand.about_desc_2')}} </span>
                                </li>
                            </ul>

                            <p class="text-gray-700 section-heading__desc">{{__('brand.about_desc_f')}}</p>

                            <div class="hidden mt-4">
                                <a href="#" class="font-semibold text-gray-900 simple-btn">
                                    {{__('brand.see_more')}}
                                    <span class="icon-right text-gradient"><i class="fas fa-arrow-right"></i></span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- ======================= About five End ========================= -->
    <!-- ======================= Service Slider Section Start ======================= -->
    <div class="overflow-hidden service-slider-section">
        <div class="flex items-center gap-4 service-slider">

            @forelse ($this->expertises as $expert)
            <div class="flex items-center gap-3 service-slider__item">
                <span class="service-slider__icon">
                    <img src="{{$expert->getFirstMediaUrl('icon')}}" class="w-12" alt="">
                </span>
                <h2 class="mb-0 font-semibold service-slider__text">{{$expert->title}}</h2>
            </div>

            @empty

            @endforelse




        </div>
    </div>
    <!-- ======================= Service Slider Section Ends ======================= -->
    <!-- ====================== property Type Five Start =============================== -->


   <livewire:home.allexpertise>
    <!-- ====================== property Type Five End =============================== -->
    <!-- ===================== Choose Us Start =========================== -->
    <section class="px-6 bg-gray-900 choose-us padding-y-120">
        <div class="container px-2 mx-auto lg:px-6">
            <div class="flex flex-col gap-4 lg:flex-row">
                <div class="w-full lg:w-6/12">
                    <div class="choose-us__thumb">
                        <div class="p-0 lg:p-4 choose-us__thumb-inner">
                            <img src="/images/house/2.png" alt="Choose Us"
                                class="h-[350px] w-full  img cover-img  object-cover">
                            <a href="https://youtu.be/QuEH0KWtePE?si=msOrdbAFR-hDUJqT"
                                class="popup-video-link video-popup__button">
                                <i class="fas fa-play"></i>
                            </a>
                        </div>
                        <img src="/assets/images/shapes/pipes.png" alt="Pipe Shape" class="pipe-shape">
                    </div>
                </div>
                <div class="order-first w-full lg:order-last lg:w-6/12 lg:pl-5">
                    <div class="choose-us__content">
                        <div class="mb-0 section-heading style-left">
                            <span class="bg-gray-700 section-heading__subtitle"> <span
                                    class="text-gradient fw-semibold">{{__('brand.why_choose_us')}}</span> </span>
                            <h2 class="text-2xl text-white lg:text-4xl section-heading__title">
                                {{__('brand.why_choose_us_title')}}</h2>
                            <p class="text-base text-justify text-gray-200 section-heading__desc fw-light">
                                {{__('brand.why_choose_us_description')}}
                            </p>
                            <div class="choose-us__button">
                                <a href="{{route('contact')}}"
                                    class="text-white btn btn-outline-lightInDark fw-normal">{{__('brand.contat_us')}}
                                    <span class="icon-right icon text-gradient">
                                        <i class="fas fa-arrow-right"></i>
                                    </span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- ===================== Choose Us End =========================== -->
    <!-- ======================= Property Section Start ===================== -->



   @livewire('propertys.propertylanding')



    <!-- ======================= Property Section End ===================== -->
    <!-- ========================== Gallery Section Start ============================== -->

    <livewire:projects.projecthome />
    <!-- ========================== Gallery Section End ============================== -->
    <!-- ======================= Counter Section Start ======================= -->
    <section class="px-6 bg-white counter-six">
        <div class="container container-two">
            <div class="bg-white px-auto counter-six__inner">
                <div class="grid grid-cols-12 gap-4 mx-auto">
                    <div class="col-span-6 lg:col-span-3">
                        <div class="counter-six-item">
                            <div class="flex items-center gap-3 lg:gap-2">
                                <span class="counter-six-item__icon">
                                    <img src="/assets/images/icons/counter-six-img1.svg" alt="">
                                </span>
                                <h2 class="mb-0 counter-six-item__number">150+</h2>
                            </div>
                            <span class="counter-six-item__text font-18">{{__('brand.projet_realiser')}}</span>
                        </div>
                    </div>
                    <div class="col-span-6 lg:col-span-3">
                        <div class="counter-six-item">
                            <div class="flex items-center gap-3 lg:gap-2">
                                <span class="counter-six-item__icon">
                                    <img src="/assets/images/icons/counter-six-img2.svg" alt="">
                                </span>
                                <h2 class="mb-0 counter-six-item__number">100+</h2>
                            </div>
                            <span class="counter-six-item__text font-18">{{__('brand.maison_vendu')}}</span>
                        </div>
                    </div>
                    <div class="col-span-6 lg:col-span-3">
                        <div class="counter-six-item">
                            <div class="flex items-center gap-3 lg:gap-2">
                                <span class="counter-six-item__icon">
                                    <img src="/assets/images/icons/counter-six-img3.svg" alt="">
                                </span>
                                <h2 class="mb-0 counter-six-item__number">50+</h2>
                            </div>
                            <span class="counter-six-item__text font-18">{{__('brand.projet_encours')}}</span>
                        </div>
                    </div>
                    <div class="col-span-6 lg:col-span-3">
                        <div class="counter-six-item">
                            <div class="flex items-center gap-3 lg:gap-2">
                                <span class="counter-six-item__icon">
                                    <img src="/assets/images/icons/counter-six-img4.svg" alt="">
                                </span>
                                <h2 class="mb-0 counter-six-item__number">20+</h2>
                            </div>
                            <span class="counter-six-item__text font-18">{{__('brand.annee_expertise')}}</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- ======================= Counter Section End ======================= -->

    <!-- ==================== Blog Start Here ==================== -->
    <section class="hidden px-6 blog padding-y-120">
        <div class="container container-two">

            <div class="section-heading">
                <div class="section-heading__inner">
                    <span class="bg-white section-heading__subtitle">
                        <span class="text-gradient fw-semibold ">Latest blog and news</span>
                    </span>
                    <h2 class="section-heading__title">Discover Endless Possibilities in Real Estate</h2>
                </div>
            </div>

            <div class="row gy-4">

                <div class="col-lg-4 col-sm-6">
                    <div class="blog-item">
                        <div class="blog-item__thumb">
                            <a href="blog-details.html" class="blog-item__thumb-link">
                                <img src="/assets/images/thumbs/blog-two-1.png" class="cover-img" alt="">
                            </a>
                            <span class="top-0 left-auto bg-white blog-item__date style-three font-20 end-0"> 23
                                <span class="text-body d-block font-16 fw-light">Mar</span> </span>

                        </div>
                        <div class="bg-white border-0 blog-item__content">
                            <ul class="p-0 border-0 text-list flx-align">
                                <li class="text-list__item font-16">
                                    <span class="icon"><i class="fas fa-user"></i></span>
                                    <a href="#" class="link">By admin</a>
                                </li>
                                <li class="text-list__item font-16">
                                    <span class="icon"><i class="fas fa-comments"></i></span>
                                    <a href="#" class="link">Comments (07)</a>
                                </li>
                            </ul>
                            <h6 class="mb-4 blog-item__title">
                                <a href="blog-details.html" class="blog-item__title-link fw-500"> Discover Endless
                                    Possibilities in Real Estate Live </a>
                            </h6>
                            <p class="mb-0 blog-item__title font-18"> Real Estate is a vast industry that deals with
                                the buying and selling.</p>

                        </div>
                    </div>
                </div>

                <div class="col-lg-4 col-sm-6">
                    <div class="blog-item">
                        <div class="blog-item__thumb">
                            <a href="blog-details.html" class="blog-item__thumb-link">
                                <img src="/assets/images/thumbs/blog-two-2.png" class="cover-img" alt="">
                            </a>
                            <span class="top-0 left-auto bg-white blog-item__date style-three font-20 end-0"> 23
                                <span class="text-body d-block font-16 fw-light">Mar</span> </span>

                        </div>
                        <div class="bg-white border-0 blog-item__content">
                            <ul class="p-0 border-0 text-list flx-align">
                                <li class="text-list__item font-16">
                                    <span class="icon"><i class="fas fa-user"></i></span>
                                    <a href="#" class="link">By admin</a>
                                </li>
                                <li class="text-list__item font-16">
                                    <span class="icon"><i class="fas fa-comments"></i></span>
                                    <a href="#" class="link">Comments (07)</a>
                                </li>
                            </ul>
                            <h6 class="mb-4 blog-item__title">
                                <a href="blog-details.html" class="blog-item__title-link fw-500"> Turn Your Real
                                    Estate Dreams Into Reality </a>
                            </h6>
                            <p class="mb-0 blog-item__title font-18"> Real Estate is a vast industry that deals with
                                the buying and selling.</p>

                        </div>
                    </div>
                </div>

                <div class="col-lg-4 col-sm-6">
                    <div class="blog-item">
                        <div class="blog-item__thumb">
                            <a href="blog-details.html" class="blog-item__thumb-link">
                                <img src="/assets/images/thumbs/blog-two-3.png" class="cover-img" alt="">
                            </a>
                            <span class="top-0 left-auto bg-white blog-item__date style-three font-20 end-0"> 23
                                <span class="text-body d-block font-16 fw-light">Mar</span> </span>

                        </div>
                        <div class="bg-white border-0 blog-item__content">
                            <ul class="p-0 border-0 text-list flx-align">
                                <li class="text-list__item font-16">
                                    <span class="icon"><i class="fas fa-user"></i></span>
                                    <a href="#" class="link">By admin</a>
                                </li>
                                <li class="text-list__item font-16">
                                    <span class="icon"><i class="fas fa-comments"></i></span>
                                    <a href="#" class="link">Comments (07)</a>
                                </li>
                            </ul>
                            <h6 class="mb-4 blog-item__title">
                                <a href="blog-details.html" class="blog-item__title-link fw-500"> Invest in Your
                                    Future with Real main Estate </a>
                            </h6>
                            <p class="mb-0 blog-item__title font-18"> Real Estate is a vast industry that deals with
                                the buying and selling.</p>

                        </div>
                    </div>
                </div>


            </div>
        </div>
    </section>
</div>
